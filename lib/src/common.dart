...
import 'package:rive_bloc/src/stack_trace.dart';

/// A utility for handling arguments with providers.
@immutable
class Args {
  /// Creates a new [Args] instance.
  const Args([Map<String, dynamic>? args]) : _args = args ?? const {};
  final Map<String, dynamic> _args;

  /// Returns the value of the argument with the given [name].
  T get<T>(String name, T valueDef) {
    if (!_args.containsKey(name)) {
      return valueDef;
    }
    return _args[name] as T;
  }

  @override
  String toString() => jsonEncode(_args);
}
...
abstract class AsyncValue<T> {
  ...
  /// }
  ///
  /// An optional callback can be specified to catch errors only under a
  /// certain condition. In the following example, we catch all exceptions
  /// beside FormatExceptions.
  ///
  /// ```dart
  ///   AsyncValue.guard(
  ///    () async { /* ... */ },
  ...
  /// provider rebuilt by [RiveBlocRef.refresh]/[RiveBlocRef.invalidate]
  /// (if true) or instead by [RiveBlocRef.watch] (if false).
...
extension AsyncValueX<T> on AsyncValue<T> {
  ...
  T get requireValue {
    ...
    throw StateError(
      'Tried to call `requireValue` on an `AsyncValue` '
  ...
  ///
  /// If there is no previous value, null will be returned during
  /// loading/error state.
  ///
  ...
  /// none of its dependencies has changed, after at least one [value]/[error]
  /// was emitted.
  ///
  /// This is usually the case when rebuilding a provider with either
  /// [RiveBlocRef.invalidate]/[RiveBlocRef.refresh].
  ///
  /// If a provider rebuilds because one of its dependencies changes (using
  /// [RiveBlocRef.watch]), then [isRefreshing] will be false, and instead
  /// [isReloading] will be true.
  bool get isRefreshing =>
      isLoading && (hasValue || hasError) && this is! AsyncLoading;

  /// Whether the associated provider was recomputed because of a dependency
  /// change (using [RiveBlocRef.watch]), after at least one [value]/[error]
  /// was emitted.
  ///
  /// If a provider rebuilds because one of its dependencies changed
  /// (using [RiveBlocRef.watch]), then [isReloading] will be true.
  ///
  /// If a provider rebuilds only due to
  /// [RiveBlocRef.invalidate]/[RiveBlocRef.refresh], then [isReloading]
  /// will be false (and [isRefreshing] will be true).
  ///
  ...
  ///
  /// Even if [hasError] is true, it is still possible for
  /// [hasValue]/[isLoading] to also be true.
  ///
  ...
  /// By default, [when] skips "loading" states if triggered by a
  /// [RiveBlocRef.refresh] or [RiveBlocRef.invalidate] (but does not skip
  /// loading states if triggered by [RiveBlocRef.watch]).
  ///
  ...
  ///   should be invoked if a provider rebuilds because of
  ///   [RiveBlocRef.watch].
  ...
  ///   should be invoked if a provider rebuilds because of
  ///   [RiveBlocRef.refresh] or [RiveBlocRef.invalidate].
  ///   In that situation, [when] will try to invoke either [error]/[data]
  ///   with the previous state.
  ///
